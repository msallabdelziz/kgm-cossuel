...
use App\Form\SearchInstallationForm;

#[Route('/installation')]
class InstallationController extends AbstractController
{
    #[Route('/', name: 'app_installation_index', methods: ['GET'])]
    public function index(Request $request,InstallationRepository $installationRepository,PaginatorInterface $page): Response
    {
        //$ins =$installationRepository->findByEtat("Saisie");
        $donnee= new SearchInstallation();
        $donnee->page = $request->get('page',1);
        $form= $this->createForm(SearchInstallationForm::class,$donnee);
        $nom = $request->get('nom');//dd($nom);
        ...
        $ins = $installationRepository->findSearch($donnee);
        return $this->render('installation/index.html.twig', [
            'les_installation' => $ins=$page->paginate(
                $ins,
    ...
    public function index2(InstallationRepository $installationRepository,PaginatorInterface $page): Response
    {
        $ins=$installationRepository->findByEtat("Soumis");
        return $this->render('installation/index.html.twig', [
            'les_installation' => $ins=$page->paginate(
                $ele,
                $request->query->getInt('page', 1),
    ...
    public function index3(InstallationRepository $installationRepository,PaginatorInterface $page): Response
    {
        $ins=$installationRepository->findByEtat("Payé");
        return $this->render('installation/index.html.twig', [
            'les_installation' => $ins=$page->paginate(
                $ele,
                $request->query->getInt('page', 1),
